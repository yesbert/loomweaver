...
1. Add the plugin to `providePlugins` in `src/app/app.config.ts`. It is **variadic** and
   ...
   ```ts
   import { ticketsPlugin } from '@loomweaver/tickets-weaver';   // Nx: the workspace alias; without one, a relative path to this library's src/index.ts
   ...providePlugins(ticketsPlugin),
   ```

2. Grant its capabilities (default-deny) via `provideCapabilityGrants`:

   ```ts
   provideCapabilityGrants({ 'tickets': ['contributions', 'ui', 'navigation'] });
   ```

3. Compose its translations with `provideTranslationNamespaces('tickets')` — and serve the
   bundle by adding an assets glob to your application's build target, so the loader can fetch
   `/i18n/tickets/<lang>.json` (the Nx generator adds this glob for you):

   ```json
   { "glob": "**/*.json", "input": "<path to this library>/src/lib/i18n", "output": "i18n/tickets" }
   ```
...
The surface is routable at `/tickets`; rail and bar items reference region ids (`primary`, `status`) that must exist in your layout.
