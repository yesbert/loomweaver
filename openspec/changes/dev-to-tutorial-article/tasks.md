...
- [ ] 2.1 Build the example first, in `examples/assistant-workbench/`: a workbench scaffolded with
...
- [ ] 2.1a A job in `.github/workflows/build.yml` beside the demo's, same shape: its own working
      directory, `npm install` from the registry, lint, build, unit tests. The deploy workflow is not
      touched; the example is not deployed.
