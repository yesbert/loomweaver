...
- [ ] 1.1 Write `docs/weaver/navigation-tree.md` with the derived-from-specs marker and the box
...
- [ ] 1.2 Section: the declaration as data, a typed array of groups and destinations for the notes
...
- [ ] 1.3 Section: reporting and acting, the `lw-nav-select` event with `detail.path`, the weaver
      ...
      `retitleSurface`
- [ ] 1.4 Section: marking where the user is, `current` fed from `ctx.activeContent`, why a deeper
      address marks the destination above it and a longer name does not
- [ ] 1.5 Section: folding, `collapsed` in the declaration and the `[attr.collapsed]` idiom, the
      `key` rule and the translated-label trap, what the session keeps and a reload forgets, and
      the group holding nothing
- [ ] 1.6 Section: hiding destinations nobody registered, from the content routes the registry
      reports, and why a product that switches a plugin off must not leave a dead entry
- [ ] 1.7 Section: retitling the panel to the area the user is in, as an effect over the marked
      destination and `ctx.retitleSurface`
- [ ] 1.8 Section: the accessible name, `aria-label` on the tree, and what the tree announces on
      its own
- [ ] 1.9 Section: content inside an item, a count after the label
- [ ] 1.10 A "Where next" that sends the reader to the recipe, to sub-routes and follows, and to
      the design-tokens bullet for the attribute table
- [ ] 1.11 Add the page to `website/sidebar.mjs` in the authoring group directly after "Surfaces
      in a sidebar"

## 2. Recipe 11 on the samples page

- [ ] 2.1 Add recipe 11, "A navigation tree in the sidebar", as whole files: the declaration
...
- [ ] 2.2 Add the anchor and the "You get" paragraph in the form the other recipes use, and add the
...
- [ ] 3.1 In `docs/reference/design-tokens.md`, shrink the sidebar-navigation bullet to the tags,
      their attributes and events and `forgetLwNavFolds()`, and point at the new page for the story
- [ ] 3.2 In `docs/weaver/sidebar-surfaces.md`, name the tree under "What your view's own body can
      use" with a link to the new page
- [ ] 3.3 In `docs/weaver/sub-routes-and-follows.md`, link the sentence "A navigation tree marks
      where the user is" to the new page

## 4. The session without a backend

- [ ] 4.1 In `docs/distribution/auth.md`, add a section before "1 · Feed the session": what
      ...
      integration
- [ ] 4.2 Replace the testbed pointer in "Shape A" with a pointer at `demo/src/session`, the
      product built on the published packages
- [ ] 4.3 Add recipe 12, "A session without a backend", as whole files: the generator's
...
- [ ] 4.4 Add recipe 12 to the generator table with `auth-source --name dev` as the invocation that
      writes its first file, and the note that the plugin is yours

## 5. The AI-readable files

- [ ] 5.1 In `llms.txt`, add the page to the weaver list with a one-line hook
- [ ] 5.2 In `llms-full.txt`, add the navigation family to the building-block bullet and the
...
- [ ] 6.1 Compile recipes 11 and 12 against the published `@loomweaver/plugin-sdk` and
...
- [ ] 6.2 `npm run docs-style-check` and `npm run api-docs-check` in `platform/`
- [ ] 6.3 `npm run sync` and `npm run check` in `website/`, so the sidebar guard and the head
