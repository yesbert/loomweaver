...
- [x] 1.1 Add `agentConsent?: 'allow' | 'ask' | 'ask-always' | 'never'` to `Command` in
...
- [x] 1.2 Add the same optional property to `InvocableCommand`, with JSDoc pointing at the
  declaration it repeats.

## 2. The enumerated account

- [x] 2.1 Carry it through `describe()` in
  `platform/libs/core/shell/src/lib/commands/command-invocation.service.ts`, untranslated (it is a
  value, not text).
- [x] 2.2 Spec test: a caller listing commands reads the statement, and a command stating nothing
  carries none.
- [x] 2.3 Spec test: the statement refuses nothing — a command declaring `ask-always`, invoked by
  identity with nobody asked, runs.
- [x] 2.4 Spec test over the sandbox rung: the statement survives `invocableCommands` crossing the
  plugin boundary.

## 3. The tool description

- [x] 3.1 `toolFor` in `platform/libs/integrations/ag-ui/src/lib/tool-definitions.ts` writes
  `metadata: { agentConsent }` where the command states something, and no `metadata` key at all where
  it does not.
- [x] 3.2 Spec test: a tool for a command that states nothing is unchanged from today; one for a
  command that states something carries it.

## 4. The seam

- [x] 4.1 `PendingToolCall` in `platform/libs/integrations/ag-ui/src/lib/command-tools.ts` gains an
...
- [x] 4.2 Spec test: `before` receives the statement for a reachable command, and `undefined` for a
...
- [x] 5.1 Read `agentConsent` off a command registration in
...
- [x] 5.2 Confirm by test that a command stating nothing neither fails strict mode nor warns, and
...
- [x] 6.1 Remove `CONSEQUENTIAL` from
...
- [x] 6.2 Update the generated comment so it explains the declaration rather than the list, and check
  the generated i18n keys for the confirm dialog still match.
- [x] 6.3 Recipe test: generated output holds no list of command identities, and the generated
...
- [x] 7.1 `llms-full.txt`: the AG-UI section's example stops teaching `DESTRUCTIVE.has(...)` and reads
...
- [x] 7.2 `docs/samples.md` sample 6 reads the statement off the call instead of keeping a set;
  `docs/ag-ui-agents.md`, `docs/scaffolding.md`, `docs/reference/agent-tools.md` and
  `docs/reference/callable-commands.md` state the declaration, its four values and its
  non-enforcement.
- [x] 7.3 Run `openspec validate --all --strict`, the affected unit suites and the repository's
