...
- [ ] 1.1 Add the root service beside `shell-features.ts` that reads `SHELL_FEATURES` once, holds
...
- [ ] 1.2 Table-driven spec: for every switch in `DEFAULT_SHELL_FEATURES`, the service reads the
...
- [ ] 1.3 Spec: `update` writes nothing to the working-state store or the settings store.
- [ ] 1.4 Lint restriction so that `SHELL_FEATURES` may be injected only in the new service and in
      `provide-shell.ts`; run lint to see it fail on the current readers before they are moved.

## 2. Every reader reads live

- [ ] 2.1 `pane-view.ts`, `pane-drop-zones.ts`, `pane-tab-strip.ts` (the copied `escalatable`
...
- [ ] 2.2 `shell-sidebar-header.ts`, `shell-panel.ts`, `curation-dialog.ts`, `shell-rail.ts`,
      `rail-workspace-entries.ts`: same.
- [ ] 2.3 `command.service.ts` (`shortcutOf` reads `commands.shortcuts()` when called),
      ...
      hint logic).
- [ ] 2.4 Adjust the existing specs of the files above where they provided `SHELL_FEATURES` as a
...
- [ ] 2.5 Rendered spec on the pane toolbar and tab strip: with every content switch on, turn
...
- [ ] 2.6 Lint is clean, including the new restriction.

## 3. Built-in seeds follow the switch

- [ ] 3.1 In `shell-seeds.ts`, wrap each switch-conditional command (`shell.content.splitRight`,
...
- [ ] 3.2 Inventory `seedBuiltInMenus` for switch-conditional menu entries and apply the same
      pattern.
- [ ] 3.3 Spec: with `splitRight` declared off, the command is absent; `update` turns it on and the
...
- [ ] 3.4 Spec for a menu-entry seed found in 3.2, same shape.

## 4. Pin the rules

- [ ] 4.1 Forward-only: split a pane, turn `splitRight` and `splitDown` off, assert the pane tree is
      unchanged and the split controls are gone.
- [ ] 4.2 Forward-only: collapse a sidebar, turn `sidebar.collapse` off, assert it stays collapsed
      and the header offers no expand control.
- [ ] 4.3 Reachable while off: turn `content.close` off, close a tab through `ContentTabsService`,
      assert it closes; with a dirty surface in it, assert the unsaved-work question is asked.
- [ ] 4.4 Reachable while off: turn `windows.popout` off, call `PopoutService.open`, assert the
      pop-out is opened.
- [ ] 4.5 Not remembered: after `update`, a fresh injector seeded from the same declaration reads the
      declaration, not the change.

## 5. Contract and documentation

- [ ] 5.1 Export the service from `@loomweaver/shell` (`index.ts`).
- [ ] 5.2 `docs/reference/host-services.md`: new section for the switches in the style of the
...
- [ ] 5.3 `docs/building-a-distribution.md`, *Switching capabilities off*: the declaration is the
      starting value; the switches can be read and changed at runtime; point at the reference.
- [ ] 5.4 Package the shell and run `check-api-docs.mjs`; every new exported name is documented.

## 6. Verify and hand over

- [ ] 6.1 `npx nx run-many -t test lint` for the shell is green; `npm run structure-check` matches
      its baseline or the baseline is adjusted for the one new concept in `foundation/`.
- [ ] 6.2 `openspec validate feature-switches-at-runtime --strict` passes.
