...
- [x] 1.1 Add `pane-handle.ts` in `regions/pane/`: the branded `PaneHandle` type, a creator the
...
- [x] 1.2 Add `PaneService` in `regions/pane/pane.service.ts` with the read side: `panes()`
...
- [x] 1.3 Spec: with one pane, one fact carrying the address; after a split, two facts with the
      right `showing` and `itemCount`, exactly one carrying the address; `isSplit()` follows; a
      `computed` on `isSplit()` re-evaluates on split.
- [x] 1.4 Spec: a handle survives a focus change and a split elsewhere; a handle of a closed pane
      resolves to nothing and every action with it is a no-op that leaves the tree unchanged.

## 2. Actions, one implementation each

- [x] 2.1 `splitRight(handle?)` / `splitDown(handle?)`: shown path of the pane, duplicate check
...
- [x] 2.2 `closePane(handle?)`: collect the unsaved-work candidates of every tab in the pane, guard,
...
- [x] 2.3 `unsplit()`: candidates of every non-primary pane, guard, then `paneTree.unsplit`. Spec:
      asked when a sibling holds work, silent when none does, tree collapses to the primary.
- [x] 2.4 `maximize(handle?)`, `minimize(handle?)`, `restore(handle?)` as explicit set/clear over the
...
- [x] 2.5 `focus(handle)`: `paneTree.focusPane` with the active tab root, then navigate to the path it
      returns. Spec: the address moves to the pane's shown item; focusing the primary is a no-op.
- [x] 2.6 `moveTab(path, handle)`: resolve the source via `paneTree.sourceOf`, then
...
- [x] 3.1 `PaneView`: `splitPane`, `closePane`, `toggleMaximize`, `minimize`, `focusPane` call the
      ...
      accordingly.
- [x] 3.2 `ContentArea`: `split`, `closePrimary`, `toggleMaximize`, `minimize` call the service for
      the primary; remove the direct tree and chrome calls. Adjust its spec.
- [x] 3.3 `shell-seeds.ts`: `shell.content.splitRight` becomes `isSplit() ? unsplit() : splitRight()`;
...
- [x] 3.4 Spec (twin): a split from the pane view, from the content area and from the service leave
      the tree in the same shape; a close from each asks the same guard with the same candidates.

## 4. Pin the rules

- [x] 4.1 Reachable while off: `content.splitRight` and `content.maximize` switched off through
      `FeatureSwitches`, the service still splits and maximises; the toolbar shows no control for
      either (rendered on `PaneView`).
- [x] 4.2 Same guard: a pane with a dirty surface closed through the service asks
      `SurfaceCloseGuard` with the same candidates the close control collects.

## 5. Contract and documentation

- [x] 5.1 Export `PaneService`, `PaneHandle` and `PaneFacts` from `@loomweaver/shell`.
- [x] 5.2 `docs/reference/host-services.md`: new section *Panes*, in the style of the neighbouring
...
- [x] 5.3 Package the shell and run `check-api-docs.mjs`: every new export documented.

## 6. Verify and hand over

- [x] 6.1 `npx nx run-many -t test lint -p shell` green; `npm run structure-check` and
      `check-import-cycles.mjs` match their baselines.
- [x] 6.2 `openspec validate pane-service --strict` passes.
