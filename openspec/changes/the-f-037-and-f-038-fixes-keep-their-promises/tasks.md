...
- [x] 2.6 The service keeps the language last switched to while its strings are there and accepts the library's own fallback only when they are not; tests for a late fallback, for a distribution without English, for another window's choice replacing one still loading, and for a stored value applying after a failed choice
