...
- [x] 1.1 Confirm the barrel export of `PluginIsolationLevel` from `@loomweaver/shell` has merged
      on its own branch; if not, do that first, with a test that the packed declaration names it
- [x] 1.2 Confirm `the-docs-explain-the-navigation-tree-and-a-session-without-a-backend` has been
      applied, so its page and its llms entries are in place
- [x] 1.3 Decide the eight names `check-api-docs` lists as visible but not exported, each on its
...
- [x] 1.4 Package `plugin-sdk` and `shell`, re-run both audits against the packed declarations and
      the current `docs/`, and mark in the lists below what has changed since 2026-09-07

## 2. The guards

- [x] 2.1 In `platform/tools/check-api-docs.mjs`, add a second pass that requires every non-exempt
...
- [x] 2.2 In `website/tools/sync-docs.mjs`, beside the sidebar check, push a problem for every page
      under `docs/` whose repo-relative path is not a link target in `llms.txt`
- [x] 2.3 Run both; the lists they print are the acceptance criterion for sections 3 to 7
- [x] 2.4 Update the guards table in `docs/reference/operations.md`: what `api-docs-check` now
      fails on, and that `sync-docs` fails on a page missing from `llms.txt`

## 3. The index, `llms.txt`

- [x] 3.1 Add the fourteen single pages of `docs/distribution-api/` under the existing overview
...
- [x] 3.2 Add `docs/README.md` and `docs/reference/operations.md`, the second with a hook that
      says it is for whoever runs or contributes to the repository
- [x] 3.3 Add `docs/reference/operations.md` to the reference list in `llms-full.txt` as well

## 4. The plugin contract in `llms-full.txt`

- [x] 4.1 In the printed `PluginContext` interface, add `retitleSurface(id, title)`,
      `isShowingUnder(path)` and `readonly state: PluginState`, each with the capability it needs
      in the trailing comment
- [x] 4.2 Add `isPreview` to the printed `PluginHost`
- [x] 4.3 Add the missing fields: `retain`, `saveOn`, `closable`, `padded`, `iframe` on `View`;
...
- [x] 4.4 Name the exports the file describes only in prose: `CAPABILITIES`, `meetsAccess`,
...
- [x] 4.5 Reword the error wording so it names `CapabilityError` as the one error class the SDK
      exports

## 5. The distribution contract in `llms-full.txt`

- [x] 5.1 Write the required-plugins block: `provideRequiredPlugins`, `REQUIRED_PLUGINS`,
      `RequiredPlugins`, what a required plugin's failure does, and add it to the "which provider"
      index
- [x] 5.2 Write the unusable-workspaces block: `UNUSABLE_WORKSPACES`, `UnusableWorkspaces`,
...
- [x] 5.3 Write the isolation-level block: `PluginIsolationLevel`, `PluginRung`,
...
- [x] 5.4 Correct the remaining signatures from the packed declaration: `provideShell` with
...
- [x] 5.5 List the services the file omits, one line each, where it says what a distribution's
...
- [x] 5.6 Name the remaining exported tokens, components and types the audit listed, or add each
...
- [x] 5.7 Add `validate-commands` to the tooling list for all three packages

## 6. The building blocks in `llms-full.txt`

- [x] 6.1 Give every tag its attributes and events, in the primitives bullet and the frame-kit
      ...
      `checked`
- [x] 6.2 Check the navigation family the sibling change added against the same standard

## 7. The capabilities the file was silent on

- [x] 7.1 Accessibility: a block from the `accessibility` spec and `docs/reference/accessibility.md`,
      what the workbench guarantees, what a plugin inherits, the checklist a plugin author owns,
      and the colour-token rule
- [x] 7.2 Product identity, the update half: installable, a failed update distinguished from
      broken offline storage, the quiet periodic check, `UpdateService` and the badge
- [x] 7.3 Shell layout: the narrow viewport and what becomes a drawer, rail names decided per rail,
      a rail that scrolls with its anchored band staying
- [x] 7.4 Routing: a deep link survives arriving before the plugin that answers it
- [x] 7.5 The mental model: one sentence that `openspec/specs/` is the contract and wins over any
      guide
- [x] 7.6 The container-query sizing rule from the design-tokens page, beside the line that tells
      an assistant to write plain CSS for sizes

## 8. The number

- [x] 8.1 Correct "27 colours" to 29 colours and 2 font families in `llms-full.txt` and in
      `docs/reference/design-tokens.md`, counted from `theme.css`

## 9. Verification

- [x] 9.1 `npm run api-docs-check` in `platform/` passes both passes with no new exemption that
      lacks a reason
- [x] 9.2 `npm run sync` and `npm run check` in `website/` pass, and the site's copies of both
      files carry the absolute links
- [x] 9.3 `npm run docs-style-check` in `platform/` for the design-tokens page
- [x] 9.4 Read both files once top to bottom for a claim the additions now contradict, in
