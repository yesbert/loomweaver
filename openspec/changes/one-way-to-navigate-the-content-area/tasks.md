...
- [ ] 1.1 Add a failing test in the content-tabs suite: content open in a second pane, an ordinary
...
- [ ] 1.2 Add a failing test that an ordinary navigation inside a pop-out window is refused and the
      window keeps its own address. It fails today; the requirement it fails is
      `popout-windows` — *A pop-out cannot stop being one*.
- [ ] 1.3 Add the test that guards the fix from overreaching: with nothing open at that address, an
...
- [ ] 2.1 In the tab layer, hand the address to the part of the arrangement already holding the
...
- [ ] 2.2 Confirm 1.1 and 1.3 now pass, and that the hand-off stays a no-op when the address is
      already carried by a part holding the content.
- [ ] 2.3 Run the whole content, pane and workspace suites. The first navigation of a session, a
...
- [ ] 3.1 Add a guard on content addresses that refuses in a window opened as a pop-out, deciding
...
- [ ] 3.2 Confirm 1.2 passes, and add the counter-test that the main window navigates normally with
      the guard installed.
- [ ] 3.3 Check that nothing else in a pop-out navigates to a content address: the deferred deep
      link and the re-navigation on a session change both run there too, and neither may now be
      refused in a loop.

## 4. Say it once, in one place

- [ ] 4.1 `docs/reference/routing.md`: replace the paragraph naming the asymmetry between a link and
...
- [ ] 4.2 `docs/building-a-distribution.md`: the sentence about navigation reaching a surface another
      pane already holds now holds for every navigation, not only the workbench's own.
- [ ] 4.3 `llms.txt`: the routing entry states the asymmetry as fact — correct it.

## 5. Hand over

- [ ] 5.1 `openspec validate --all --strict` and the repository's own guards for the touched project.
- [ ] 5.2 Open the pull request naming both halves: the new `routing` requirement, and the
