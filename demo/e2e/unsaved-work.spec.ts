...
/* The close control is a sibling of the tab button, not a child of it, so it is reached through
   the wrapper the two share. On a tab holding unsaved work it also shares its slot with the mark
   and appears once the pointer is over the tab, which is why the wrapper is hovered first. */
