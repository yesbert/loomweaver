...
See [how a file actually gets created](#how-a-file-actually-gets-created) below. This section is what
the server does; the path from a registered server to a running product, with a run recorded as it
happened, is [Building with an AI assistant](building-with-an-assistant.md).
