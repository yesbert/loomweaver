...
- Tokens + theme: **one source**, `@loomweaver/shell/styles/theme.css`.
...
(injected into `lw-plugin-theme` as `:root`/`:root.dark`, **never** inline styles, which would beat
...
measurement that _is_ a contract is the shared bar-control height (see below), and it exists because
it drifted twice without one.
...
content unless you say `min-w-0`. A chart canvas reports a content width, so a card holding one will
hold its column open no matter what the breakpoints say. And a surface presented as an isolated
...
document, and it receives the shell's compiled stylesheet and the token push, not yours. And a size
...
dimension tokens today, and there will be none until a second product asks for the same handful,
because one data point names a set that fits one design rather than the set products need.
...
`bg-*`, `text-*`, `border-*`. Use **only** these names for colour, never raw hex or palette values.
...
`tooltip` / `tooltip-content` are reserved for `<lw-tooltip>`.

### Typography

`--lw-font-sans` (the UI font, driving `body` plus Tailwind's `font-sans` utility) and `--lw-font-mono`
(code) are theme tokens as well: a theme or a tenant can set the brand font through
`ctx.contributeTheme` or tenant CSS, like any colour. **Font _size_ is deliberately NOT a token but a
user setting** (the built-in "text size" under Settings → Options scales the whole UI via the `:root`
`font-size`). It is not part of the theme contract, so branding and accessibility stay cleanly
separated.
...
- **Icons:** `<lw-icon name="add" size="1rem" />`. Names are semantic rather than pictorial;
  ...
  installed plugin cannot repaint the chrome. Contributed SVG is **sanitised** at registration
  (DOMPurify, SVG profile: `<script>`, event handlers and `javascript:` are removed, and markup that
  does not survive is discarded). A weaver then references the name in its contributions
  (`icon: 'name'`) **or** draws it directly by tag. **`<lw-icon>` is a framework-agnostic custom element**: it resolves the name
...
- **Tooltip (`<lw-tooltip>`):** a **framework-agnostic custom element**, like `<lw-icon>`, and
  **usable inside your own weaver body** (by tag, no `@loomweaver/shell` import; in an Angular weaver set
  ...
  hover** (like a native `title`, appearing where the mouse is, which matters for wide triggers such
...
- **Menus/context menus (`<lw-menu>`):** on **host chrome** a weaver does **not** draw the menu itself.
  ...
  **icon + shortcut hint**; **`checkedWhen`** turns it into a **`menuitemcheckbox`** (e.g. "Pinned ✓" instead of a Pin/Unpin pair). **The exception is your own sandbox surface:** there the plugin
...
- **From an SDK-only plugin (`scope:weaver`):** the `lwButton` directive lives in the shell and is
  **not** importable across the Nx boundary. Two framework-free ways, both on the same `.lw-btn`
  ...
  padding and font, so `lw-btn lw-btn--<variant>` alone already sits correctly) plus one variant
  ...
  is **no** `lw-btn--md`, because `md` is the default.
- **Icon buttons in chrome (`.lw-icon-btn`):** the shared core of the subtle icon buttons in host
  ...
  utility decision at the call site** (`h-6 w-6`, `rounded-md`, …), and the contract only supplies
  tone + hover in one place. For labelled buttons keep using `.lw-btn`.
- **Segmented selection (`.lw-segmented` / `.lw-segmented-item`):** a container with one button per
  ...
  _is_ the style hook, and there is no `--active` class. **Pitfall:** `.lw-segmented-item` is by
  contract a **square icon field** (7×7); for text segments override the size at the call site
  (`w-auto px-3 text-sm`), because size stays per-site utilities, as with `.lw-icon-btn`.
  **Shared bar-control height:** `.lw-segmented` and `.lw-select-trigger` both pin `h-8.5`
  (34px), so controls standing next to each other in a bar are the same height by construction, and
  new bar controls should carry the same height.
- **Text fields (`.lw-field`):** put the class **`lw-field`** on a **native** `<input>` / `<textarea>` /
  `<input type="date">`, and it themes through the tokens (tenant-/theme-reactive, like `.lw-btn`).
  ...
  encapsulate non-native elements, a text field only needs styling, not a web component.
...
- **Radio / slider (`.lw-radio` / `.lw-range`):** class contracts on a **native** `<input type="radio">`
  ...
  **`slider`** (a number with `min`/`max`/`step`) renders `.lw-range` + a value readout.
...
- **Divider / collapsible (`.lw-divider` / `.lw-collapsible`):** class contracts on a **native** `<hr>`
  ...
  JS**; the class tints header + body (`.lw-collapsible-body`) and replaces the marker with a chevron that
  rotates on `[open]`.
- **The pattern for future `<lw-*>` primitives:** wherever a host building block should also be usable
  ...
  the contract consumable across the Nx boundary without a `@loomweaver/ui` import kit.
...
- **`<lw-markdown source="…">`** is a **framework-agnostic custom element** (like `<lw-tooltip>`/`<lw-select>`/`<lw-menu>`): it renders Markdown as **sanitised** HTML (via `marked` +
  ...
  **`prose-lw`** utility (in `theme.css`), which maps the `--tw-prose-*` colours onto the `--lw-*`
...
The fix: `position: fixed` plus CSS anchor positioning, with `anchor-name` on the trigger and
`position-anchor` + `position-area` on the popup. `<lw-select>` is the worked example: the element
sets `anchor-name` and `position-anchor`, and its `position-area` lives in `theme.css`.
...
**in JS** (at the cursor on mouse hover, otherwise at the element, with no CSS anchor positioning).
If a popup must be a child of a potentially `transform`ed ancestor, take the same route; otherwise the
...
- `no-unknown-classes` (**error**) reports mistyped or non-existent utilities. It ignores `^lw-` and
  Angular structure like `ng-*`.
- `enforce-consistent-class-order` (**warn**) keeps the class order stable, and is `--fix`-able.

The plugin resolves the class registry from the entry point named in its ESLint settings, which is
the stylesheet that imports Tailwind and the shell's theme.
