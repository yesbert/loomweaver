...
| The adapter  | `@loomweaver/ag-ui` | The logic on the workbench side. `list()` turns the commands the plugin may reach into protocol tools, with the declared arguments as JSON Schema. `receive(event)` assembles a call from its events, asks your hook, runs the command and answers with a tool message. Once the run is over, `flush()` answers each call it left open, one per request, until it answers nothing; such a call is refused, not run. |
