...
export function readme(distribution: ResolvedDistribution): string {
  return [
    ...
    '',
    '```sh',
    `nx serve ${distribution.name}    # in an Nx workspace`,
    'ng serve                          # in an Angular CLI application',
    '```',
    '',
    "The files keep Angular's own shape: `main.ts` bootstraps `App`, `App` renders `<lw-shell />`, and",
    'everything this product is made of lives in `app.config.ts`. Nothing of the generated app is',
    'deleted. Over an existing application `--force` replaces exactly the files above, all of them',
    'bootstrap wiring and none of them content you authored. Whether the build around them was wired',
    'as well depends on how they arrived; see "Build wiring" below.',
    '',
    'Two leftovers from `ng new` are no longer referenced and can go: `src/app/app.routes.ts` (the',
    'shell owns content routing via `provideShellRouter()`) and `src/app/app.css`. The starter test',
    ...
    '## Build wiring',
    '',
    'The build target needs the stylesheet, three asset globs, the service worker and two production',
    'settings. Who added them depends on how these files arrived:',
    '',
    '- **The Nx generator** wrote them into the project\'s `project.json`. Composing over an existing',
    '  project with `--force`, it merged them in value by value: every value the project already had',
    '  stays, and what the workbench needs is added beside it.',
    '- **The CLI inside an Angular CLI application** added what was missing to the build target in',
    '  `angular.json` and listed each line it added. Anything you had already set was left as you set',
    '  it.',
    '- **The MCP server**, and the CLI anywhere else, cannot reach your build configuration, so they',
    '  returned these as steps still to do by hand.',
    '',
    ...
    '',
    'The initial-bundle budget in your build target was raised for the same reason: the shell is a',
    "whole application chrome and spends most of Angular's default before your own code arrives, so",
    'a production build would have failed on a number meant for an empty app. It is still a budget,',
    ...
    'Two switches are finer, and named for it: `splitRightButton` and `splitDownButton` leave out the',
    "pane toolbar's split button alone, for a product that offers splitting by dragging a tab to a",
    "pane's edge. The command and the tab menu's entry are removed by naming them in",
    '`provideShell({ omit })` instead, so each decision keeps one place.',
